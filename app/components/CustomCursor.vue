...
<template>
  <div v-if="!isTouch" class="fixed top-0 left-0 z-9999 pointer-events-none md:block hidden">
    ...
    <!-- YOU label: same transform, but with a lag via CSS transition -->
    <span
      class="absolute z-0 top-0 left-0 px-3 py-1.5 rounded-full bg-secondary text-white text-xs font-bold whitespace-nowrap shadow-md transition-[transform,opacity] duration-300 ease-out"
      :class="isVisible ? 'opacity-100' : 'opacity-0'" :style="{ transform: `translate(${x + 16}px, ${y + 30}px)` }">
      YOU
    </span>
  </div>
</template>