...
const facebook = 'https://www.facebook.com/kentoy.newt'
const linkedin = 'https://www.linkedin.com/in/kent-joemar-escoto-646b92265'
const instagram = 'https://www.instagram.com/_itskjle/'
...
function handleClickOutside(event: MouseEvent) {
  if (
    isSocialsOpen.value &&
    socialsRef.value &&
    !socialsRef.value.contains(event.target as Node)
  ) {
...
<template>
  <div class="fixed top-0 left-0 right-0 z-50 px-6 py-6 sm:px-10 md:px-16 lg:px-20 transition-colors duration-300"
    ...
    <div class="flex items-center gap-4" :class="[isMaintenance ? 'justify-between' : 'justify-center']">
      <a href="/" class="w-fit h-fit shrink-0">
        <ClientOnly>
          <img src="/meta/red_icon.png" alt="KinWebb Logo"
            class="size-8 sm:size-10 inline-block active:scale-95 duration-200 hover:opacity-80" />
        </ClientOnly>
      </a>

      <div v-if="isMaintenance" class="relative" ref="socialsRef">
        <button class="block sm:hidden cursor-pointer" @click.stop="showSocials()">
          <svg v-if="!isSocialsOpen" xmlns="http://www.w3.org/2000/svg"
            class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 24 24">
          ...
          <svg v-else xmlns="http://www.w3.org/2000/svg"
            class="size-7 shrink-0 opacity-40 hover:opacity-30 active:scale-95 duration-200" viewBox="0 0 24 24">
            <path d="M0 0h24v24H0z" fill="none" />
        ...
        <section
          class="flex sm:flex-row flex-col gap-2 sm:gap-4 items-center absolute sm:static top-10 right-0 sm:top-auto">
          <a v-for="(social, index) in [
            { href: dribbble, key: 'dribbble' },
            { href: github, key: 'github' },
            { href: facebook, key: 'facebook' },
            { href: instagram, key: 'instagram' },
            { href: linkedin, key: 'linkedin' },
          ]" :key="social.key" :href="social.href" target="_blank"
            class="transition-all ease-out sm:opacity-100! sm:translate-y-0! sm:duration-0!"
            :class="isSocialsOpen ? 'opacity-100 translate-y-0 duration-200' : 'opacity-0 -translate-y-2 duration-150 pointer-events-none sm:pointer-events-auto'"
            :style="isSocialsOpen ? `transition-delay: ${index * 60}ms` : `transition-delay: 0ms`">

            <svg v-if="social.key === 'dribbble'" xmlns="http://www.w3.org/2000/svg"
              class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
              <g fill="none">
                <path
                ...
                <path fill="currentColor"
                  d="M13.774 14.439a32.8 32.8 0 0 1 1.536 7A10 10 0 0 1 12 22a9.95 9.95 0 0 1-5.567-1.692a12.03 12.03 0 0 1 7.34-5.87Zm7.799.463a10.02 10.02 0 0 1-4.355 5.63a35 35 0 0 0-1.46-6.468a12.06 12.06 0 0 1 5.815.838m-9.428-4.417q.499 1.018.93 2.073a14.03 14.03 0 0 0-8.207 6.451a10.08 10.08 0 0 1-2.827-7.922c3.343.708 6.87.502 10.104-.602m7.575-4.841a10.07 10.07 0 0 1 2.24 7.26a14.1 14.1 0 0 0-6.9-.77a35 35 0 0 0-1.055-2.398a19 19 0 0 0 5.715-4.092M7.266 3.19a33 33 0 0 1 3.93 5.502a17.13 17.13 0 0 1-8.776.43A10.03 10.03 0 0 1 7.266 3.19M12 2c2.388 0 4.581.837 6.3 2.234a17 17 0 0 1-5.22 3.726a35 35 0 0 0-3.86-5.568A10 10 0 0 1 12 2" />
              </g>
            </svg>

            <svg v-if="social.key === 'github'" xmlns="http://www.w3.org/2000/svg"
              class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
              <path fill="currentColor" fill-rule="evenodd"
            ...
            <svg v-if="social.key === 'facebook'" xmlns="http://www.w3.org/2000/svg"
              class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
              <path fill="currentColor"
            ...
            <svg v-if="social.key === 'instagram'" xmlns="http://www.w3.org/2000/svg"
              class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
              <path fill="currentColor"
            ...
            <svg v-if="social.key === 'linkedin'" xmlns="http://www.w3.org/2000/svg"
              class="size-7 shrink-0 hover:opacity-80 active:scale-95 duration-200" viewBox="0 0 16 16">
